...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert .txt files to audio (StyleTTS2) OR convert PDFs to .txt."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["styletts2"],
        help="TTS engine to use (StyleTTS2 is the only engine)",
    )
    parser.add_argument(
        "--voice",
        default="default",
        help="StyleTTS2: 'default' for the built-in voice, or a path to a reference clip to clone from",
    )
    parser.add_argument(
        "--ref-audio",
        dest="ref_audio",
    ...
    parser.add_argument("--speaker", default=None, help="Optional alias for --voice preset")
    parser.add_argument("--language", default=None, help="Optional language code, e.g. en, en-gb, es, fr")
