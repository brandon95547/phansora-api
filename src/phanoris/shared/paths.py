...
code lives inside the installed ``phanoris`` package, ``__file__`` points *into*
...
``WorkingDirectory``) or the ``PHANORIS_DATA_DIR`` env var.
"""
...
def runtime_root() -> Path:
    ...
    Defaults to the current working directory; override with ``PHANORIS_DATA_DIR``.
    """
    return Path(os.getenv("PHANORIS_DATA_DIR", os.getcwd())).resolve()
