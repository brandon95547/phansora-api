"""Unified Phanoris API application.

Each product ships a self-contained FastAPI app (``phanoris.products.<name>``).
...
product's optional heavy dependencies (torch, faiss, styletts2, asyncpg, ...)
...
Run with:  uvicorn phanoris.main:app --host 0.0.0.0 --port 8000
"""
...
logger = logging.getLogger("phanoris")

# prefix -> "module:attr" of the product's FastAPI app
_PRODUCT_APPS = {
    "/spokenverse": "phanoris.products.spokenverse.server:app",
    "/chrono": "phanoris.products.chrono_origin.server:app",
    "/dossier": "phanoris.products.dossier_nova.api:app",
}
...
def _load_products() -> Dict[str, FastAPI]:
    ...
    for prefix, target in _PRODUCT_APPS.items():
        if enabled and _PREFIX_TO_KEY[prefix] not in enabled:
            logger.info("Skipping %s (not in PHANORIS_ENABLED_PRODUCTS)", prefix)
            continue
...
if __name__ == "__main__":
    ...
    uvicorn.run("phanoris.main:app", host=settings.host, port=settings.port, reload=True)
