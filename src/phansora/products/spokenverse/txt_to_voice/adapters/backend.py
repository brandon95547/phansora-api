...
GPT-SoVITS is the sole engine. The selector is kept as a thin indirection so the
...
_GPTSOVITS_ALIASES = {"gptsovits", "gpt-sovits", "gpt_sovits", "sovits", "gsv", "default", ""}
# Engines that used to exist here; a leftover TTS_ENGINE / --engine pointing at one
# degrades to GPT-SoVITS with a warning rather than crashing every request.
_RETIRED_ALIASES = {"styletts2", "styletts-2", "style", "stts2", "st2", "kokoro", "openvoice", "chatterbox"}
...
def resolve_engine(engine: str | None = None) -> str:
    ...
    if name in _RETIRED_ALIASES:
        if name not in _warned_retired:
            LOG.warning(
                "TTS engine '%s' has been removed; using GPT-SoVITS. "
                "Update TTS_ENGINE / --engine to 'gptsovits' to silence this.",
                name,
            )
            _warned_retired.add(name)
        return "gptsovits"
    # Everything else (gptsovits aliases, unknown values) resolves to the only engine.
    return "gptsovits"


def _module(engine: str | None):
    resolve_engine(engine)  # validate (warns on retired engines)
    from . import gptsovits_client as mod  # type: ignore
    return mod
