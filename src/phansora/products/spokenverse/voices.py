"""Per-user custom voices for Chatterbox voice cloning.

A "voice" is a short reference audio clip the user uploads; Chatterbox clones it
...
MAX_SECONDS = 90
# Chatterbox reference clips: 24 kHz mono.
_SAMPLE_RATE = 24000
...
# Per-voice generation knobs, mirroring the options Chatterbox supports. These are
# captured at approval and reapplied when the voice is later used for TTS.
from phansora.products.spokenverse.txt_to_voice.adapters.chatterbox_client import (
    EXAGGERATION_MIN, EXAGGERATION_MAX, EXAGGERATION_DEFAULT,
    CFG_WEIGHT_MIN, CFG_WEIGHT_MAX, CFG_WEIGHT_DEFAULT,
    TEMPERATURE_MIN, TEMPERATURE_MAX, TEMPERATURE_DEFAULT,
    MIN_P_MIN, MIN_P_MAX, MIN_P_DEFAULT,
    TOP_P_MIN, TOP_P_MAX, TOP_P_DEFAULT,
    REPETITION_PENALTY_MIN, REPETITION_PENALTY_MAX, REPETITION_PENALTY_DEFAULT,
)

_KNOBS = (
    ("exaggeration", EXAGGERATION_MIN, EXAGGERATION_MAX, EXAGGERATION_DEFAULT),
    ("cfg_weight", CFG_WEIGHT_MIN, CFG_WEIGHT_MAX, CFG_WEIGHT_DEFAULT),
    ("temperature", TEMPERATURE_MIN, TEMPERATURE_MAX, TEMPERATURE_DEFAULT),
    ("min_p", MIN_P_MIN, MIN_P_MAX, MIN_P_DEFAULT),
...
def clamp_settings(
    ...
    min_p=None, top_p=None, repetition_penalty=None,
) -> dict:
    """Coerce/clamp Chatterbox generation knobs to supported ranges, filling defaults."""
    given = {
    ...
    out = {}
    for name, lo, hi, default in _KNOBS:
        try:
            v = float(given[name]) if given[name] is not None else default
        except (TypeError, ValueError):
            v = default
        out[name] = max(lo, min(hi, round(v, 2)))
    return out
...
def pending_path(user_id: str, token: str) -> Optional[Path]:
    """The pending reference clip (what Chatterbox clones from)."""
...
def save_pending_settings(user_id: str, token: str, **knobs) -> dict:
    """Record the generation knobs used for a pending sample so approval can persist
    ...
    if p.exists():
        try:
            ...
            if isinstance(data, dict):
                return clamp_settings(**{k: data.get(k) for k, *_ in _KNOBS})
        except Exception:
            pass
...
def list_voices(user_id: str) -> List[dict]:
    ...
    for v in _load_manifest(user_id):
        if voice_path(user_id, v.get("id", "")) is None:
            continue
        out.append({**v, **clamp_settings(**{k: v.get(k) for k, *_ in _KNOBS})})
    return out
