...
|---|---|---|
| **SpokenVerse** | `/spokenverse` | PDF→text (render + OCR + AI cleanup), text→audio (StyleTTS2 voice cloning), audio→text (faster-whisper), and **Book Alchemy** (durable, Postgres-backed book→audio-course pipeline) |
...
SpokenVerse (StyleTTS2) and Dossier Nova (sentence-transformers) both need
